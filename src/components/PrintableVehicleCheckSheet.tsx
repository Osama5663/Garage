...
import { VehicleCheckSheet } from '../types/jobOrder'
...
interface PrintableVehicleCheckSheetProps {
  title: string
  ...
  jobNumber: string
  sheet: VehicleCheckSheet
}

export const PrintableVehicleCheckSheet: React.FC<PrintableVehicleCheckSheetProps> = ({
  title,
  referenceNumber,
  date,
  recipient,
    ...
    <div className="bg-white p-8 max-w-4xl mx-auto print-container">
      ...
      <div className="mb-6 bg-gray-50 rounded-lg p-4 border border-gray-200">
        <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
          ...
          {typeof sheet.mileage === 'number' && <div><span className="text-gray-600">Kilométrage:</span> <span className="font-semibold">{sheet.mileage} km</span></div>}
          {sheet.fuel && <div><span className="text-gray-600">Carburant:</span> <span className="font-semibold">{sheet.fuel}</span></div>}
          {sheet.firstRegistrationDate && <div><span className="text-gray-600">Mise en circulation:</span> <span className="font-semibold">{new Date(sheet.firstRegistrationDate).toLocaleDateString()}</span></div>}
        </div>
      </div>
      ...
      {sheet.globalNote && (
        <div className="mb-12 border-t border-gray-200 pt-6">
          ...
          <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{sheet.globalNote}</p>
        </div>
      )}
