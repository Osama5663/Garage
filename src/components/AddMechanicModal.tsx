...
const AddMechanicModal: React.FC<AddMechanicModalProps> = ({ onClose }) => {
  ...
  const { addMechanic } = useMechanicStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setErrors({});
    addMechanic({ name, specialization, phone, status: 'active', hourlyRate: Number(hourlyRate) });
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Ajouter un mécanicien</h2>
